...
const premiumseats = document.querySelectorAll(
  ".row .premium-seat:not(.booked"
);
...
premiumContainer.addEventListener("click", (e) => {
  if (
    e.target.classList.contains("premium-seat") &&
...
function refreshPage() {
  ...
  const premiumbookedSeats = JSON.parse(
    localStorage.getItem("premiumbookedSeats")
  );
...
function premiumUserSelectedSeat() {
  const premiumselectedseats = document.querySelectorAll(
    ".row .premium-seat.selected"
  );
...
function useralreadyselectedPremiumseat() {
  const alreadyselectedPremiumSeats = document.querySelectorAll(
    ".row .premium-seat.selected"
  );
